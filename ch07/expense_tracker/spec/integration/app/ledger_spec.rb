    ...
    describe '#record' do
      context 'with a valid expense' do
        pit 'successfully saves the expense in the DB' do
          ...
          expect(DB[:expenses].all).to match [a_hash_including(id: result.expense_id, payee: 'Starbucks', amount: 5.75, date: Date.iso8601('2017-06-10'))]
        end
      end
      ...
      context 'when the expense lack a date' do
        it 'rejects the expense as invalid' do
          expense.delete('date')
